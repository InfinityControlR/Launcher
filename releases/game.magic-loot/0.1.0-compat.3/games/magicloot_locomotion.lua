...
function Module.create(context)
    ...
    local function stopBroomApproach()
        if type(context.stopBroomApproach) == "function" then
            pcall(context.stopBroomApproach)
        end
    end

    local function approachBroomGate(stage)
        if type(context.approachBroomGate) ~= "function" then return nil end
        local ok, moved, detail = pcall(context.approachBroomGate, stage)
        if not ok or moved ~= true then return nil end
        return tostring(detail or "broom approaching stage entrance")
    end
    ...
    local function updateBroom()
        ...
        local approachStatus = approachBroomGate(selected)
        if not broom.delayStarted then
            scheduleBroomDelay(now, broom.delayDuration)
        end
        if now < broom.readyAt then
            broom.status = string.format(
                "%s; stage %d armed in %.1fs",
                approachStatus or "broom ready at base",
                selected,
                broom.readyAt - now
            )
            return
    ...
    -- World Event entry must use the same post-character-controller ownership
    -- as Walking. A sparse Humanoid:MoveTo request can be overwritten every
    -- frame by Roblox's character controller, which leaves the player stopped
    -- at base even though the event objective already owns movement.
    function api:UpdateEventApproach(root, destination)
        if root == nil or typeof(destination) ~= "Vector3" then
            resetAll()
            return false, "event approach waiting for destination"
        end
        local character = root.Parent
        local humanoid = character and character:FindFirstChildOfClass("Humanoid")
        if humanoid == nil or humanoid.Health <= 0 then
            resetAll()
            return false, "event approach waiting for character"
        end

        local now = os.clock()
        local distance = planarDistance(destination, root.Position)
        if distance <= 4 then
            stopMovement()
            return true, "event approach arrived"
        end

        local sameRoute = state.active
            and state.mode == "EventApproach"
            and state.humanoid == humanoid
            and state.root == root
        if not sameRoute then
            resetAll()
            ...
            if not moved then
                resetAll()
                return false, "event approach unavailable"
            end
            state.bound = true
            startWatchdog(state.generation)
        end

        -- The live door waypoint changes to the confirmed interior waypoint
        -- without yielding movement ownership between both legs.
        state.destination = destination
        state.finalDestination = destination
        state.heartbeat = now
        return true, string.format("event approach %.1f studs", distance)
    end

    -- World Event uses the same native Running controller around a fixed arena
    -- center supplied by the game adapter instead of a stage footprint.
    -- Render-step owns smooth movement and the existing grounded 0.9 s jump
    -- cadence; dragon replication never moves the orbit itself.
    function api:UpdateEventRunning(root, center)
        if root == nil or typeof(center) ~= "Vector3" then
            resetAll()
            return "event running waiting for center"
        end
        local character = root.Parent
        local humanoid = character and character:FindFirstChildOfClass("Humanoid")
        if humanoid == nil or humanoid.Health <= 0 then
        ...
        state.heartbeat = now
        return string.format(
            "event running %.1f studs from center; waypoint %.1f studs",
            state.orbitRadius,
            distance
        )
    end
