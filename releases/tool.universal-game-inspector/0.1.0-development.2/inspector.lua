...
-- artifact-version: 0.1.0-development.2
...
function Inspector:start()
	...
	self._starting = false
	if self._privacyInventoryFailed then attachOk = false end
...
function Layout.compact(rect)
	local width = math.min(520, math.max(1, rect.width - MARGIN * 2))
	local height = math.min(168, math.max(88, math.floor(rect.height * 0.28)))
	if width * height > rect.width * rect.height * 0.30 then
		height = math.floor(rect.width * rect.height * 0.30 / width)
	end
	return {
		width = width,
...
local SENTINEL_IDENTITY = "game-discovery-inspector-artifact|0.1.0-development.2"
...
local function launchInternal()
	...
	local dragHandle = newInstance("TextButton", {
		...
		TextColor3 = Color3.fromRGB(245, 248, 255), TextScaled = true, Font = Enum.Font.GothamBold,
	...
	local minimizeButton = newInstance("TextButton", {
		...
		TextScaled = true, Font = Enum.Font.GothamBold, Active = true,
	}, frame)
	...
	local function actionButton(name, text, index)
		local value = newInstance("TextButton", {
			...
			TextColor3 = Color3.fromRGB(255,255,255), TextScaled = true, Font = Enum.Font.GothamBold, Active = true,
		}, frame)
		owned[value] = true
		return value
	end
	...
	local pageHelp = newInstance("TextLabel", {
		...
		TextColor3 = Color3.fromRGB(245, 220, 150), TextScaled = true, Font = Enum.Font.Gotham,
	}, details)
	owned[pageHelp] = true
	local pageText = newInstance("TextBox", {
		...
		Text = "Detén la captura y pulsa Exportar/Copiar.", ClearTextOnFocus = false, MultiLine = true,
		TextEditable = true, TextWrapped = false, TextXAlignment = Enum.TextXAlignment.Left,
		TextYAlignment = Enum.TextYAlignment.Top, Font = Enum.Font.Code, TextSize = 12, Active = true,
	}, details)
	owned[pageText] = true
	...
	local floatingButton = newInstance("TextButton", {
		...
		TextScaled = true, Font = Enum.Font.GothamBold, Active = true, Visible = false,
	}, screen)
	...
	local function viewportSize()
		...
		if cameraOk and camera ~= nil then
			...
			if sizeOk and size ~= nil then return size.X, size.Y end
		end
		return 360, 640
	end
	...
	local function setDisplay(nextState, code)
		appState = nextState
		...
		if code ~= nil then pipeline.lastError = code end
		if nextState == "error" and frame.Parent ~= nil then
			detailsOpen, details.Visible = true, true
			applyLayout()
		end
	end
	local function refreshProgress()
		if inspector ~= nil then
			...
			if ok and type(value) == "table" then records, duration = value.record_count or records, value.duration_seconds or duration end
		end
		setDisplay(appState)
	end
	...
	local function showPage(index)
		...
		if not retained.ok then
			pcall(function() pageText.Text = "" end)
		...
		local ok = pcall(function()
			pageText.Text = pipeline.pages[pageIndex]
	...
	local function cleanup()
		...
		if inspector ~= nil then
			...
			if not ok or type(result) ~= "table" or result.ok ~= true then
				unloading = false setDisplay("error", "DISCOVERY_CLEANUP_FAILED") warn("DISCOVERY_CLEANUP_FAILED") return false
			end
			inspector = nil
		end
		pipeline:clear()
	...
	connect(startButton.Activated, function()
		if captureState ~= "ready" then setDisplay("error", "DISCOVERY_CONFIG_INVALID") refreshDiagnostic() return end
		pipeline:clear()
					...
					return task.delay(seconds, function()
						...
						if active and inspector ~= nil and inspector:state() == "stopped" then
							...
							if exported.ok then
								...
								if sealed then setDisplay("stopped") else expireInspectorIfNeeded(sealError) setDisplay("error", sealError) end
							else setDisplay("error", exported.error.code) end
							refreshDiagnostic()
						end
					end)
				end,
		...
		if not created.ok then setDisplay("error", created.error.code) refreshDiagnostic() return end
		inspector = created.value
		local started = inspector:start()
		if not started.ok then setDisplay("error", started.error.code) refreshDiagnostic() return end
	...
	connect(stopButton.Activated, function()
		...
		if captureState ~= "capturing" or inspector == nil then setDisplay("error", "DISCOVERY_CONFIG_INVALID") refreshDiagnostic() return end
		local stopped = inspector:stop()
		refreshProgress()
		if not stopped.ok then setDisplay("error", stopped.error.code) refreshDiagnostic() return end
		captureState = "stopped"
		local exported = inspector:exportSnapshot()
		if not exported.ok then setDisplay("error", exported.error.code) refreshDiagnostic() return end
		local sealed, sealError = pipeline:seal(exported.value, exported.retention_expires_clock_us)
		if not sealed then expireInspectorIfNeeded(sealError) setDisplay("error", sealError) refreshDiagnostic() return end
		setDisplay("stopped") refreshDiagnostic()
	end)
	connect(exportButton.Activated, function()
		if captureState == "stopped" then
			local retained = pipeline:validateRetention()
			if not retained.ok then expireInspectorIfNeeded(retained.code) setDisplay("error", retained.code) refreshDiagnostic() return end
		end
		if captureState ~= "stopped" or pipeline.snapshot == nil then setDisplay("error", "DISCOVERY_CONFIG_INVALID") refreshDiagnostic() return end
		local encoded = pipeline:encode()
		if not encoded.ok then expireInspectorIfNeeded(encoded.code) setDisplay("error", encoded.code) refreshDiagnostic() return end
		local textReady, textError = showPage(1)
		local delivered = pipeline:deliverClipboard()
		if not delivered.ok then expireInspectorIfNeeded(delivered.code) setDisplay("error", delivered.code) refreshDiagnostic() return end
		...
		elseif textReady then
			...
			pipeline:_fail(textError, "textbox") setDisplay("error", textError)
		end
		refreshDiagnostic()
	end)
	...
	connect(previousButton.Activated, function()
		local ok, code = showPage(pageIndex - 1) if not ok then
			...
			expireInspectorIfNeeded(code) setDisplay("error", code)
	...
	connect(selectButton.Activated, function()
		local retained = pipeline:validateRetention()
		if not retained.ok then expireInspectorIfNeeded(retained.code) setDisplay("error", retained.code) refreshDiagnostic() return end
		local ok = pcall(function() pageText:CaptureFocus() pageText.SelectionStart = 1 pageText.CursorPosition = #pageText.Text + 1 end)
		if not ok then pipeline:_fail("DISCOVERY_ARTIFACT_TEXTBOX_FOCUS_FAILED", "textbox") setDisplay("error", "DISCOVERY_ARTIFACT_TEXTBOX_FOCUS_FAILED") end
		retained = pipeline:validateRetention()
		if not retained.ok then expireInspectorIfNeeded(retained.code) setDisplay("error", retained.code) end
		refreshDiagnostic()
	end)
	connect(chunksButton.Activated, function()
		local result = pipeline:printChunks(print)
		if result.ok then setDisplay("exported") else expireInspectorIfNeeded(result.code) setDisplay("error", result.code) end refreshDiagnostic()
	end)
	...
	if runService ~= nil then connect(runService.Heartbeat, function()
		...
		elseif captureState == "stopped" then
			local retained = pipeline:validateRetention()
			if not retained.ok then expireInspectorIfNeeded(retained.code) setDisplay("error", retained.code) refreshDiagnostic() end
		end
	end) end
