...
-- artifact-version: 0.1.0-development.5
...
local function attachSignal(inspector, surface, signal, callback, privacyOwned)
	if signal == nil then
		return false, "unavailable"
	end
	if not inspector:_transitionCanStart() then return false, "timeout" end
	...
	local owned = if privacyOwned then inspector:_ownPrivacyConnection(connection) else inspector:_ownConnection(connection, surface)
	local completed = inspector:_transitionCompleted()
	if not completed then return false, "timeout" end
	if not owned then return false, "limit" end
	return true, nil
end
...
local function observeLocalContainer(inspector, container, surface)
	...
	local function visit(instance, operation)
		...
		if inspector:_surfaceRequested("value_bases") and inspector:_privacySensitiveCollectorsAllowed() then
...
function Observers.attach(inspector, dependencies)
	...
	local protectedCharacters = setmetatable({}, { __mode = "k" })
	local privacyInventoryHealthy = true
	local privacyLocalPlayer = nil
	local function degradePrivacy(status, reason)
		privacyInventoryHealthy = false
		inspector:_privacyInventoryDegraded(status, reason)
		return false
	end
	local function privacyRead(reader, reason)
		if not inspector:_transitionCanStart() then
			return nil, degradePrivacy("partial", "timeout")
		end
		local ok, value = pcall(reader)
		local completed = inspector:_transitionCompleted()
		if not completed then return nil, degradePrivacy("partial", "timeout") end
		if not ok then return nil, degradePrivacy("unavailable", reason) end
		return value, true
	end
	local function privacyAttach(signal, callback, reason)
		local attached, attachReason = attachSignal(inspector, "errors", signal, callback, true)
		if attached then return true end
		local status = attachReason == "timeout" and "partial" or "unavailable"
		return degradePrivacy(status, reason .. "_" .. tostring(attachReason))
	end
	local function protectPlayerIdentity(player)
		if player == nil or not privacyInventoryHealthy then return false end
		local isPlayer, isPlayerOk = privacyRead(function() return player:IsA("Player") end, "player_type")
		if not isPlayerOk then return false end
		if not isPlayer then return true end
		if protectedPlayers[player] then return true end
		protectedPlayers[player] = true
		for _, field in ipairs({ "Name", "DisplayName" }) do
			local function protectField()
				local value, readOk = privacyRead(function() return player[field] end, "player_field")
				if not readOk then return false end
				if not inspector:_addPrivateName(value) then return degradePrivacy("partial", "identity_limit") end
				return true
			end
			if not protectField() then return false end
			local signal, signalOk = privacyRead(function()
				return player:GetPropertyChangedSignal(field)
			end, "player_field_signal")
			if not signalOk or not privacyAttach(signal, protectField, "player_field_signal") then return false end
		end
		local character, characterOk = privacyRead(function() return player.Character end, "player_character")
		if not characterOk then return false end
		local function protectCharacter(value)
			if value == nil then return true end
			if protectedCharacters[value] then return true end
			protectedCharacters[value] = true
			local function protectCharacterName()
				local name, nameOk = privacyRead(function() return value.Name end, "character_name")
				if not nameOk then return false end
				if not inspector:_addPrivateName(name) then return degradePrivacy("partial", "identity_limit") end
				return true
			end
			if not protectCharacterName() then return false end
			local signal, signalOk = privacyRead(function()
				return value:GetPropertyChangedSignal("Name")
			end, "character_name_signal")
			if not signalOk or not privacyAttach(signal, protectCharacterName, "character_name_signal") then return false end
			return true
		end
		if not protectCharacter(character) then return false end
		local characterAdded, characterAddedOk = privacyRead(function() return player.CharacterAdded end, "character_added_signal")
		if not characterAddedOk or not privacyAttach(characterAdded, protectCharacter, "character_added_signal") then
			return false
		end
		return true
	end
	if players == nil then
		degradePrivacy("unsupported", "players_service")
	else
		local roster, rosterOk = privacyRead(function() return players:GetPlayers() end, "roster_read")
		if rosterOk and type(roster) == "table" then
			for index, rosterEntry in ipairs(roster) do
				if index > inspector:_scanLimit() then degradePrivacy("partial", "roster_limit") break end
				if not protectPlayerIdentity(rosterEntry) then break end
			end
		elseif rosterOk then
			degradePrivacy("unavailable", "roster_shape")
		end
		if privacyInventoryHealthy then
			local candidate, candidateOk = privacyRead(function() return players.LocalPlayer end, "local_player")
			if candidateOk then
				privacyLocalPlayer = candidate
				protectPlayerIdentity(candidate)
			end
		end
		if privacyInventoryHealthy then
			local playerAdded, playerAddedOk = privacyRead(function() return players.PlayerAdded end, "player_added_signal")
			if playerAddedOk then privacyAttach(playerAdded, protectPlayerIdentity, "player_added_signal") end
		end
	end
	if privacyInventoryHealthy then inspector:_privacyInventoryReady() end
	local privacySensitiveAllowed = inspector:_privacySensitiveCollectorsAllowed()
	local privacySensitiveStarted = false
	...
	if players ~= nil then
		localPlayer = privacyLocalPlayer
		if localPlayer == nil then
			local localPlayerOk, candidate = pcall(function() return players.LocalPlayer end)
			if localPlayerOk then localPlayer = candidate end
		end
		if localPlayer ~= nil then
			inspector:_setLocalPlayer(localPlayer)
			local characterOk, localCharacter = pcall(function() return localPlayer.Character end)
			if characterOk then inspector:_setLocalCharacter(localCharacter) end
			privacySensitiveAllowed = inspector:_privacySensitiveCollectorsAllowed()
			if inspector:_surfaceRequested("character") then
				inspector:_markSurfaceStarted("character")
				if characterOk and localCharacter ~= nil then
					observeCharacter(inspector, localCharacter, runService)
				end
			...
			if privacySensitiveAllowed and playerGui ~= nil and inspector:_surfaceRequested("ui") then
				inspector:_markSurfaceStarted("ui")
				observeUiTree(inspector, playerGui)
				privacySensitiveStarted = true
			elseif privacySensitiveAllowed and inspector:_surfaceRequested("ui") then
				inspector:_markSurfaceUnavailable("ui", "local_player_gui_unavailable")
			end
			local backpack = localPlayer:FindFirstChildOfClass("Backpack")
			if backpack ~= nil and (inspector:_surfaceRequested("inventory")
				or (privacySensitiveAllowed and inspector:_surfaceRequested("value_bases")))
			then
			...
			if leaderstats ~= nil and (inspector:_surfaceRequested("stats")
				or (privacySensitiveAllowed and inspector:_surfaceRequested("value_bases")))
			then
	...
	local rootScanRequested = privacySensitiveAllowed and (inspector:_surfaceRequested("hierarchy") or inspector:_surfaceRequested("instances")
	...
	if privacySensitiveAllowed and privacySensitiveStarted then inspector:_markPrivacySensitiveObserversActive() end
	local supported = {
	...
	for surface, requested in pairs(inspector:_requestedSurfaces()) do
		if not inspector:_transitionCanStart() then break end
		if requested and inspector:_isPrivacySensitiveSurface(surface) and not privacySensitiveAllowed then
			-- The exact fixed privacy_inventory_* reason was set before any such
			-- observer could attach or buffer data.
		elseif requested and supported[surface] then
...
local PRIVACY_SENSITIVE_SURFACES = table.freeze({
	hierarchy = true,
	instances = true,
	attributes = true,
	value_bases = true,
...
function Inspector.new(inputConfig, dependencies)
	...
	self._privacyInventoryStatus = "pending"
	self._privacyInventoryReason = nil
	self._privacySensitiveObserversActive = false
...
-- An optional collector that returns after its budget cannot be preempted in
-- Luau. Its late return is isolated from the required collectors by starting a
-- fresh required-attachment budget. The optional collector is degraded and is
-- never retried automatically.
function Inspector:_renewTransitionAfterOptionalCollector()
	local ok, now = pcall(self._transitionClockUs)
	if not ok or type(now) ~= "number" or now ~= now then return false end
	self._transitionDeadlineUs = math.floor(now) + TRANSITION_DEADLINE_US
	self._transitionTimedOut = false
	return true
end
...
function Inspector:_privacyInventoryDegraded(status, reason)
	if status ~= "unsupported" and status ~= "unavailable" and status ~= "partial" then
		status = "unavailable"
	end
	self._privacyInventoryStatus = status
	self._privacyInventoryReason = reason
	for surface in pairs(PRIVACY_SENSITIVE_SURFACES) do
		if self:_surfaceRequested(surface) then
			self:_markSurfaceUnavailable(surface, "privacy_inventory_" .. status)
		end
	end
	if self._starting then
		if self._transitionTimedOut then self:_renewTransitionAfterOptionalCollector() end
		if not self:_releasePrivacyConnections() then
			return self:_fault("DISCOVERY_CLEANUP_FAILED", "privacy.inventory.cleanup")
		end
		if self:_surfaceRequested("errors") then
			self:_emit("errors", "error", {
				category = "unknown",
				path_segments = { "privacy", "inventory" },
				class_name = "PrivacyInventory",
				value_type = "enum",
			}, { state = status }, {})
		end
		return false
	end
	-- Once privacy-sensitive observers have started, their prior buffer may
	-- already contain a newly introduced identity. Continuing would expose data,
	-- so runtime degradation remains fail-closed for the whole capture.
	if self._privacySensitiveObserversActive and self._state == "observing" then
		return self:_fault("DISCOVERY_INTERNAL_ERROR", "privacy.inventory.runtime")
	end
	if not self:_releasePrivacyConnections() then
		return self:_fault("DISCOVERY_CLEANUP_FAILED", "privacy.inventory.cleanup")
	end
...
function Inspector:_privacyInventoryReady()
	self._privacyInventoryStatus = "available"
	self._privacyInventoryReason = nil
	return true
end

function Inspector:_privacySensitiveCollectorsAllowed()
	return self._privacyInventoryStatus == "available"
end

function Inspector:_isPrivacySensitiveSurface(surface)
	return PRIVACY_SENSITIVE_SURFACES[surface] == true
end

function Inspector:_markPrivacySensitiveObserversActive()
	self._privacySensitiveObserversActive = true
end

function Inspector:_addPrivateName(name)
	return self._sanitizer:addEphemeralIdentity(name)
end
...
function Inspector:_setLocalCharacter(character)
	...
	if character ~= nil then
		...
		if ok then
			if not self:_addPrivateName(name) then self:_privacyInventoryDegraded("partial", "identity_limit") end
		else
			self:_privacyInventoryDegraded("partial", "local_character_read")
		end
	end
end
...
function Inspector:_ownPrivacyConnection(connection)
	if not self:_ownConnection(connection, "errors") then return false end
	table.insert(self._privacyConnections, connection)
	return true
end

function Inspector:_releasePrivacyConnections()
	if #self._privacyConnections == 0 then return true end
	local released = setmetatable({}, { __mode = "k" })
	local remainingPrivacy = {}
	for _, connection in ipairs(self._privacyConnections) do
		local ok = pcall(function() connection:Disconnect() end)
		if ok then
			released[connection] = true
		else
			table.insert(remainingPrivacy, connection)
		end
	end
	local retained = {}
	for _, connection in ipairs(self._connections) do
		if not released[connection] then table.insert(retained, connection) end
	end
	self._connections = retained
	self._privacyConnections = remainingPrivacy
	return #remainingPrivacy == 0
end
...
function Inspector:start()
	...
	-- An optional privacy collector may have returned after its deadline and
	-- opened a fresh budget for independent required collectors.
	deadline = self._transitionDeadlineUs or deadline
	local attachCompleted = self:_transitionCompleted(deadline)
	self._starting = false
	if self._lastFault ~= nil then attachOk = false end
...
function ExportPipeline:encode()
	...
	if self.json ~= nil and self.hash ~= nil and #self.pages > 0 and #self.chunks > 0 then
		if self.lastError ~= "DISCOVERY_ARTIFACT_JSON_PRIMARY_FAILED" then
			self.lastError, self.phase = nil, nil
		end
		self.state = "encoded"
...
function ExportPipeline:deliverClipboard(prepared)
	if prepared == true then
		local retained = self:validateRetention()
		if not retained.ok then return retained end
		if self.json == nil or self.hash == nil or #self.pages == 0 then
			return self:_fail("DISCOVERY_ARTIFACT_SNAPSHOT_INVALID", "export.clipboard.prepare")
		end
	else
	...
	for _, name in ipairs({ "setclipboard", "clipboard" }) do
		...
		if type(backend) == "function" then
			...
			if ok and result ~= false then
				if self.lastError == "DISCOVERY_ARTIFACT_CLIPBOARD_UNAVAILABLE" or self.lastError == "DISCOVERY_ARTIFACT_CLIPBOARD_FAILED" then self.lastError, self.phase = nil, nil end
...
local SENTINEL_IDENTITY = "game-discovery-inspector-artifact|0.1.0-development.5"
...
local function launchInternal()
	...
	local function snapshotReachedCapacity()
		local snapshot = pipeline.snapshot
		return type(snapshot) == "table"
			and type(snapshot.truncation) == "table"
			and snapshot.truncation.occurred == true
			and type(snapshot.limits) == "table"
			and type(snapshot.limits.raw_queue_records) == "number"
			and type(snapshot.records) == "table"
			and #snapshot.records >= snapshot.limits.raw_queue_records
	end
	local function setSnapshotDisplay(nextState)
		if snapshotReachedCapacity() then
			setDisplay(nextState, "DISCOVERY_LIMIT_REACHED", "capture.capacity")
		else
	...
	connect(startButton.Activated, function()
		if captureState ~= "ready" then setDisplay("error", "DISCOVERY_CONFIG_INVALID", "lifecycle.start.state") refreshDiagnostic() return end
		pipeline:clear()
					...
					return task.delay(seconds, function()
						...
						if active and inspector ~= nil and inspector:state() == "stopped" then
							...
							if exported.ok then
								...
								if sealed then setSnapshotDisplay("stopped") else expireInspectorIfNeeded(sealError) setDisplay("error", sealError, pipeline.phase) end
	...
	connect(stopButton.Activated, function()
		...
		setSnapshotDisplay("stopped") refreshDiagnostic()
	end)
	connect(exportButton.Activated, function()
		...
		if pipeline.snapshot == nil then setDisplay("error", "DISCOVERY_CONFIG_INVALID", "lifecycle.export.state") refreshDiagnostic() return end
		...
		local delivered = pipeline:deliverClipboard(true)
		...
		if textReady then
			detailsOpen, details.Visible = true, true applyLayout()
		end
		if delivered.backend == "clipboard" or textReady then
			setSnapshotDisplay("exported")
		else
